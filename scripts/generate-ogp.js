...
async function generateOGPImage() {
  try {
    ...
    const twitterSummarySvgPath = path.join(__dirname, '../public/images/twitter-summary.svg');
    const twitterSummaryPngPath = path.join(__dirname, '../public/images/twitter-summary.png');
    
    if (!fs.existsSync(twitterSummarySvgPath)) {
      console.error('Twitter summary用SVGファイルが見つかりません:', twitterSummarySvgPath);
      return;
    }
    ...
    console.log('✅ Twitter summary用画像を生成しました:', twitterSummaryPngPath);
    
    const twitterSummaryStats = fs.statSync(twitterSummaryPngPath);
    console.log(`📊 Twitter summary用ファイルサイズ: ${(twitterSummaryStats.size / 1024).toFixed(2)} KB`);
